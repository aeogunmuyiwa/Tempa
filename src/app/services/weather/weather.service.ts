...
export class WeatherService {
  ...
  getCityWeatherByName(city: string, metric: 'metric' | 'imperial' = 'metric'): Subject<string> {
    ...
    this.http.get(
      `https://api.openweathermap.org/data/2.5/weather?q=${city}&units=${metric}&APPID=ef125c41bb79578e91dd5f430a31a577`)
      .subscribe((data) => {
  ...
  getWeatherState(city: string): Subject<string> {
    ...
    this.http.get(
      `https://api.openweathermap.org/data/2.5/weather?q=${city}&APPID=ef125c41bb79578e91dd5f430a31a577`)
      .subscribe((data) => {
  ...
  getCurrentTemp(city: string, metric: 'metric' | 'imperial' = 'metric'): Subject<number> {
    ...
    this.http.get(
      `https://api.openweathermap.org/data/2.5/weather?q=${city}&units=${metric}&APPID=866439dc285bbdd41fc13fc2706c1928`)
  ...
  getMaxTemp(city: string, metric: 'metric' | 'imperial' = 'metric'): Subject<number> {
    ...
    let max: number;
    this.http.get(
      `https://api.openweathermap.org/data/2.5/forecast?q=${city}&units=${metric}&APPID=ef125c41bb79578e91dd5f430a31a577`)
      .subscribe((weather: any) => {
